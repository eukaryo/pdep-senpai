...
template<int kind, int sparseness>void bench_pext_sparse() {

	static_assert(kind == __naive || kind == __intrinsics || kind == __table_16_16, "invalid kind");
	static_assert(-10 <= sparseness && sparseness <= 10, "invalid sparseness");

	std::cout << "Bench pext:" << (kind == __naive ? "naive" : (kind == __intrinsics ? "intrinsics" : "table_16_16")) << ", spaeseness = " << sparseness << std::endl;
	uint64_t result = 0;
	uint64_t a = 0x1111222233334444ULL;
	uint64_t mask = 0x5555666677778888ULL;
	uint64_t rand = 0x5555666677778888ULL;

	auto start = std::chrono::system_clock::now();
	for (int i = 0; i < (1 << 30); ++i) {
		result ^= kind == __naive ? pext_naive(a, mask) : (kind == __table_16_16 ? pext_table_16_16(a, mask) : pext_intrinsics(a, mask));
		a = xorshift64(a);
...
int main() {
	...
	bench_pext_sparse<__table_16_16, 0>();
	bench_pext_sparse<__intrinsics, 0>();
	bench_pext_sparse<__table_16_16, 1>();
	bench_pext_sparse<__intrinsics, 1>();
	bench_pext_sparse<__table_16_16, 2>();
	bench_pext_sparse<__intrinsics, 2>();
	bench_pext_sparse<__table_16_16, -1>();
	bench_pext_sparse<__intrinsics, -1>();
	bench_pext_sparse<__table_16_16, -2>();
	bench_pext_sparse<__intrinsics, -2>();
